...
use claude_usage::config::{Config, get_config};
use std::env;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Batch Size (from env): {}", config_with_env.processing.batch_size);
    println!("   Memory Limit (from env): {}MB", config_with_env.memory.max_memory_mb);
    println!("   Log Level (from env): {}\n", config_with_env.logging.level);
    ...
    println!("   Example: get_config().processing.batch_size = {}", 
