...
impl PricingManager {
    ...
    async fn fetch_pricing_data() -> Result<HashMap<String, PricingData>> {
        ...
        let response = client
            .get(url)
            .header("User-Agent", &format!("claude-usage/{}", env!("CARGO_PKG_VERSION"))) // Identify ourselves
            .send()
            .await?;
            ...
            for (model_name, pricing_data) in pricing_obj {
                if model_name.starts_with("claude-") {
                    let pricing = PricingData {
                        input_cost_per_token: pricing_data
                            .get("input_cost_per_token")
                            .and_then(|v| v.as_f64()),
                        output_cost_per_token: pricing_data
                            .get("output_cost_per_token")
                            .and_then(|v| v.as_f64()),
                        cache_creation_input_token_cost: pricing_data
                            .get("cache_creation_input_token_cost")
                            .and_then(|v| v.as_f64()),
                        cache_read_input_token_cost: pricing_data
